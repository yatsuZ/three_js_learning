...
export class SandboxPhysics {
    constructor(scene, soundManager) {
        ...
        this.world.addContactMaterial(new CANNON.ContactMaterial(this.defaultMaterial, groundMaterial, { friction: 0.3, restitution: 0.5 }));
        this.world.addContactMaterial(new CANNON.ContactMaterial(this.defaultMaterial, this.defaultMaterial, { friction: 0.3, restitution: 0.5 }));
        // Sol
        const floorMesh = new THREE.Mesh(new THREE.PlaneGeometry(30, 30), new THREE.MeshStandardMaterial({ color: 0x333333, roughness: 0.8 }));
    ...
    createWalls() {
        ...
        [
            ...
            if (pos.rotY)
                body.quaternion.setFromEuler(0, pos.rotY, 0);
    ...
    spawnObject(shape, pos, breakable, restitution) {
        if (!this.canSpawn())
            return;
        let mesh;
        let cannonShape;
        ...
        const obj = { mesh, body, breakable, health: 100 };
    ...
    breakObject(obj) {
        ...
        const color = obj.mesh.material.color.getHex();
        this.scene.remove(obj.mesh);
        this.world.removeBody(obj.body);
        for (let i = 0; i < 5; i++) {
            const size = 0.15 + Math.random() * 0.15;
            const debrisMesh = new THREE.Mesh(new THREE.BoxGeometry(size, size, size), new THREE.MeshStandardMaterial({ color, roughness: 0.6 }));
