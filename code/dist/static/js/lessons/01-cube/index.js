import { createScene, setupResize, CubeManager, addLights } from "../../shared/index.js";
...
function createCubes(savedRotations) {
    ...
    CUBES_CONFIG.forEach((config, index) => {
        const cube = cubeManager.createSingleCube(Object.assign(Object.assign({}, config), { size: 0.8 }));
...
function toggleLighting(enabled) {
    ...
    if (enabled && !lights) {
        ...
        if (lights.ambient)
            ctx.scene.remove(lights.ambient);
        if (lights.point)
            ctx.scene.remove(lights.point);
        if (lights.directional)
            ctx.scene.remove(lights.directional);
        lights = null;
    }
