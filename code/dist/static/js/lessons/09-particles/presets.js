 ...
 */
export function generatePreset(preset, config, positions, colors, data) {
    switch (preset) {
        case 'galaxy':
 ...
 * Galaxie spirale
 */
function generateGalaxy(config, positions, colors) {
    const branches = 5;
    const spin = 1;
 ...
 * Neige qui tombe
 */
function generateSnow(config, positions, colors, velocities) {
    for (let i = 0; i < config.count; i++) {
        const i3 = i * 3;
 ...
 * Feu qui monte
 */
function generateFire(config, positions, colors, velocities, lifetimes) {
    for (let i = 0; i < config.count; i++) {
        const i3 = i * 3;
 ...
 * Fontaine d'eau
 */
function generateFountain(config, positions, colors, velocities, lifetimes) {
    for (let i = 0; i < config.count; i++) {
        const i3 = i * 3;
        const angle = Math.random() * Math.PI * 2;
 ...
 * Explosion
 */
function generateExplosion(config, positions, colors, velocities, lifetimes, initialPositions) {
    for (let i = 0; i < config.count; i++) {
        const i3 = i * 3;
        positions[i3] = 0;
        positions[i3 + 1] = 0;
