...
import type { ParticlePreset } from './ui.ts';
...
export class SandboxParticles {
	...
	private preset: ParticlePreset = 'galaxy';

	constructor(scene: THREE.Scene) {
		this.scene = scene;
	}

	create(preset: ParticlePreset, count: number): void {
		this.dispose();
		this.preset = preset;
	...
	private initParticle(i: number, positions: Float32Array, colors: Float32Array, preset: ParticlePreset): void {
		const i3 = i * 3;
	...
	update(delta: number): void {
		if (!this.geometry || !this.velocities || !this.lifetimes || !this.points) return;
