...
import { getUIElements, type UIElements, type ModeType, type ShapeType, type ParticlePreset } from './ui.ts';
import { SoundManager } from './sound.ts';
import { SandboxParticles } from './particles.ts';
import { SandboxPhysics } from './physics.ts';
...
class Lesson12 extends LessonBase {
	...
	private mode: ModeType = 'physics';

	private soundManager!: SoundManager;
	private physics!: SandboxPhysics;
	private particles!: SandboxParticles;
	...
	constructor() {
		const config: LessonConfig = { id: '12', name: 'Sandbox Final' };
		super(config);
	}
	...
	private setupEventListeners(): void {
		this.addEventListener(this.ui.mode, 'change', () => this.setMode(this.ui.mode.value as ModeType));
		...
		this.addEventListener(this.ui.particlePreset, 'change', () => {
			this.particles.create(this.ui.particlePreset.value as ParticlePreset, parseInt(this.ui.particleCount.value));
		...
		this.addEventListener(this.canvas, 'click', (e) => this.onCanvasClick(e as MouseEvent));
	}

	private setMode(mode: ModeType): void {
		this.mode = mode;
	...
	private spawnObject(): void {
		...
		this.physics.spawnObject(
			this.ui.shape.value as ShapeType,
			pos,
			this.ui.breakableToggle.checked,
			parseFloat(this.ui.restitution.value)
		);
	}

	private onCanvasClick(event: MouseEvent): void {
		if (this.mode !== 'physics' || !this.physics.canSpawn()) return;
