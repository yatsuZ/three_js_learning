...
function createTexturedCube(texture: THREE.Texture | null, position: { x: number; y: number; z: number }): THREE.Mesh {
...
function setupCubes(): void {
	...
	(cubes[3].material as THREE.MeshStandardMaterial).color.set('#888888');
}
...
function setupUpload(): void {
	const input = document.getElementById('texture-upload') as HTMLInputElement;
	input.addEventListener('change', (e) => {
		...
		reader.onload = (event) => {
			...
			img.onload = () => {
				...
				const material = cubes[3].material as THREE.MeshStandardMaterial;
